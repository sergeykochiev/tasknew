...
const HeaderTab: FC<{ children: string, redirectTo?: string, router?: AppRouterInstance, light: boolean }> = ({ children, redirectTo, router, light }) => {
    ...
    return <div className="flex group">
        ...
        <label className="h-full flex items-end">
            <div className={`transition-all  header-tab-content w-[100px] h-full group-has-[input:checked]:h-[90%] select-none cursor-pointer group-has-[input:checked]:cursor-default text-[14px] grid place-items-center ${light ? "group-has-[input:checked]:bg-white" : "group-has-[input:checked]:bg-bg-dark"} ${light ? "group-has-[input:checked]:text-bg-dark" : "group-has-[input:checked]:text-main-gray"} group-has-[input:checked]:font-bold rounded-t-[16px] hover:font-bold`}>
                {children}
            </div>
