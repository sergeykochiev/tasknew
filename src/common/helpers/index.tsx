...
export function apply2pxHover(elementId: string) {
    ...
    if (element) {
        ...
        element.addEventListener("mouseover", () => {
            console.log(element.children[0] as HTMLElement);
            // element.style.height = `${height + 4}px`
            // element.style.marginTop = "-2px"
            // element.style.marginBottom = "-2px"
            // element.style.width = `${width + 4}px`
            (element.children[0] as HTMLElement).style.transform = `scaleX(${scaleX}) scaleY(${scaleY})`
        })
        element.addEventListener("mouseleave", () => {
            ...
            (element.children[0] as HTMLElement).style.transform = "scale(1)"
        })
    }
}